...
function  [ObjectsThisOrganoid] = New_f_ImageAnalysis_IBA1FOXA2MAP2(Label, ch1, ch2, ch3, ch4, ChannelNames, PreviewPath);
...
%% FOXA2 (ch4)
    ...
    FOXA2Mask = bwareaopen(FOXA2Mask, 500);%vol(FOXA2Mask)
    %bwareaopen excludes from the mask what is smaller than the specified
    %number of pixels
   ...
   ch1MedFilt = []; 
    ...
    IBA1Mask = ch1MedFilt > 40; % vol(IBA1Mask)
...
% Scalebar
    ...
    PreviewIBA1 = imoverlay2(imadjust(max(ch1,[],3),[0 0.02]), bwperim(max(IBA1Mask,[],3)), [0 1 0]);
    PreviewIBA1 = imoverlay2(PreviewIBA1, BarMask, [1 1 1]);
    %imtool(PreviewIBA1)
    
    PreviewFOXA2 = imoverlay2(imadjust(max(ch4,[],3),[0 0.01]), bwperim(max(FOXA2Mask,[],3)), [1 0 0]);
